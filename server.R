...
#proportion of males with 1 successful nest attempts
...
shinyServer(function(input,output,session){
  ...
  pveca <- reactive({c(ps1a(),ps2a(),ps3a())})
  Aa <- reactive ({t(c(1,2,3))%*%(pveca())}) #mean number of successful nest attempts, adults
  Fa <- reactive({input$ya*(input$Ea/2)*Aa()*input$fa*input$ChMort2a*input$fMort2a}) #male young per Adult male
  ...
  pvecs <- reactive({c(ps1(),ps2(),ps3())})
  As <- reactive ({t(c(1,2,3))%*%(pvecs())}) #mean number of nest attempts, adults
  Fs <- reactive({input$ys*(input$E/2)*As()*input$f*input$ChMort2*input$fMort2}) #male young per subAdult male
  ...
  Fa2 <- reactive({input$ya2*(input$Ea2/2)*Aa2()*input$fa2*input$ChMort2a2*input$fMort2a2}) #male young per Adult male
  ...
  As2 <- reactive ({t(c(1,2,3))%*%(pvecs2())}) #mean number of nest attempts, adults
  Fs2 <- reactive({input$ys2*(input$E2/2)*As2()*input$f2*input$ChMort22*input$fMort22}) #male young per Adult male
  ...
  output$errcheck <- renderText({
    paste("ps1a : ",round(ps1a(),3),"ps2a :",round(ps2a(),3),"ps3a :",round(ps3a(),3))
  })
